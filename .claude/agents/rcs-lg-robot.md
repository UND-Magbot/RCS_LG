...
| LTE 라우터 `TC700_5170086` (**로봇용**) | 로봇 192.168.39.100 · WAN 10.115.244.12 |
| LTE 라우터 `TC700_5168495` (**서버PC·태블릿용**) | PC 192.168.39.110 · WAN 10.115.244.11 |

> ⚠️ **2026-09-18 정정** — 위 두 줄이 서로 반대로 적혀 있었다(이 파일 32행과도 모순).
> 실제 매핑은 **5170086=로봇 / 5168495=서버**이며 2026-08-21 현장 확인 사항이다
> (`docs/02_네트워크.md`, `docs/06_테스트현황과_이슈.md:343`, `docs/07_LGIT현장설치_LTE.md`).
>
> ⚠️ **라우터 2대의 LAN 이 둘 다 `192.168.39.1`, 대역도 둘 다 `192.168.39.x`** 다.
> 태블릿에 두 SSID 가 모두 저장돼 있으면 안드로이드가 신호 센 쪽으로 자동 전환하는데,
> **로봇 라우터는 로봇에 얹혀 같이 돌아다닌다.** 로봇이 가까이 오면 태블릿이 그쪽에
> 붙어버리고, 그래도 `192.168.39.x` 를 받으므로 화면엔 "연결됨" 으로 보이지만
> 서버(`.110`)는 그 LAN 에 없어 **모든 요청이 실패**한다.
> → 태블릿에는 **서버 라우터 SSID 하나만** 저장할 것.
